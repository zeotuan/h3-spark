...
case class GetBaseCellNumber2(child: Expression) extends UnaryExpression with ImplicitCastInputTypes with NullIntolerant {
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    val H3Class = H3.getClass.getName.stripSuffix("$")
    nullSafeCodeGen(
      ctx,
      ev,
      origin => {
        s"""
         |${ev.value} = ${H3Class}.getInstance().getBaseCellNumber($origin);
  ...
  override protected def withNewChildInternal(newChild: Expression): GetBaseCellNumber2 = copy(child = newChild)
}
